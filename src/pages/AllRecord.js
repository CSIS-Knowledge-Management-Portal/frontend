...
function AllRecord() {
    ...
    try {
      const data = await axios.get(
        `${process.env.REACT_APP_ROOT_URL}/api/recordtype/`,
        {
          ...
          params: {
            recordtype: id, //in mode 0 send id
            mode: 1,
          },
        }
      );
  ...
  const deleteRecord = async () => {
    try {
      await axios.delete(`${process.env.REACT_APP_ROOT_URL}/api/recordtype`, {
        headers: { Authorization: localStorage.getItem("SavedToken") },
        data: {
          id: id,
        },
    ...
    <>
      <Flex gap="md" direction="column" wrap="wrap" sx={{ padding: 10 }}>
        <Flex
          ...
          sx={{ padding: 4 }}
        >
          <Title order={3}>{records?.name}</Title>
          <Button.Group>
            <Button
              color="white"
              variant="contained"
              ...
              onClick={() =>
                navigate("/update-recordtype", {
                  state: { recordTypeData: records },
                })
              }
            >
              Edit
